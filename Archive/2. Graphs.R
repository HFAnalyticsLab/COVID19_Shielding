...
# SPL_by_LA_All <- as.data.table(SPL_by_LA_All)
# 
# pct_shielding_by_income_dep_quintile <- SPL_by_LA_All[, list(
#   Shielders_pct=weighted.mean(Shielders_pct,pop19)), 
#   by = list(quintile_income)] %>%
#   filter(.,!is.na(quintile_income))
# 
# cols <- brewer.pal(n = 5, name = "RdYlGn")
# deplabs <- c("1 (most deprived)",2:4,"5 (least deprived)")
# 
# as_tibble(pct_shielding_by_income_dep_quintile)
# 
# by_deprivation_quintile <- ggplot(pct_shielding_by_income_dep_quintile) +
#   geom_chicklet(aes(x=factor(quintile_income), y = Shielders_pct,fill=factor(quintile_income)),
#                 radius = grid::unit(5, 'mm'),width = 0.75) +
#   geom_text(aes(x = quintile_income, y = Shielders_pct + 0.2,
#                 label = paste0(round(Shielders_pct, 1),"%")),
...
#         panel.background = element_blank(),axis.text.y=element_blank(),
#         axis.ticks.y=element_blank(),axis.ticks.x=element_blank(),
#         text = element_text(family = "quicksand", size = 40),
...
# #Save chart
# 
# ggsave(paste0(gitdir,"/Charts/","by_deprivation_quintile.png"), by_deprivation_quintile, device="png",width = 8, height = 4,dpi=500)
...
filter(SPL_by_LA_All,LA.Name=="Wales")
...
NI <- as.data.frame(t(c(80000/(1.882*1000000)*100,"Northern Ireland")))
